...
func (s *Server) addNewFollower(c echo.Context) error {
	...
	if payload.UserId == uid {
		return c.JSON(400, newResponse(nil, "you can't follow yourself"))
	}
	...
	if err == nil || check_follow.ID != uuid.Nil {
		return c.JSON(400, newResponse(nil, "you already followed this user"))
	}
...
func (s *Server) removeFollowing(c echo.Context) error {
	...
	if check_following.ID == uuid.Nil || err != nil {
		return c.JSON(400, newResponse(nil, fmt.Sprintf("[%v] is not in your followings list!", follow_id)))
	}
